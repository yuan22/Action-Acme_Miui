...
cat<<EOT
Usage:
${0##*/} SRC_DIR OUTPUT_FILE [-s] [-m MOUNT_POINT] [-d PRODUCT_OUT] [-C FS_CONFIG ] [-c FILE_CONTEXTS] [-z COMPRESSOR] [-T TIMESTAMP] [-U UUID] [-b max-extent-bytes]
EOT
}
...
OPT=""
if [ -n "$extent" ]; then
  OPT="$OPT --max-extent-bytes $extent"
fi
...
MAKE_EROFS_CMD="./bin/mkfs.erofs $COMPRESSOR $OPT $OUTPUT_FILE $SRC_DIR"
echo $MAKE_EROFS_CMD
$MAKE_EROFS_CMD
