...
sudo find ./** |while read line
do
sudo chown -hR $myuser:$myuser "$line"
sudo chmod 777 "$line"
done

sed -i 's/\x2C\x76\x65\x72\x69\x66\x79/\x00\x00\x00\x00\x00\x00\x00/g' split_img/boot.img-zImage
sed -i 's/\x2C\x61\x76\x62/\x00\x00\x00\x00/g' split_img/boot.img-zImage
sed -i 's/user/user androidboot.verifiedbootstate=green androidboot.selinux=permissive/g' split_img/boot.img-cmdline

if [ -d "ramdisk" ]; then
for line in $(grep "<fs_mgr_flags>" ramdisk/*.* -rn -l ); do
chmod 0777 $line
